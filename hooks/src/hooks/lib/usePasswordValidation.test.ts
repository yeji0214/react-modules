...
describe("카드 비밀번호 입력에 대한 usePasswordValidation 커스텀 훅 테스트", () => {
  it("모든 조건이 유효할 때, isValid는 true여야 한다", () => {
    const { result } = renderHook(() =>
      usePasswordValidation({ password: "54" })
    );
  ...
  it("비밀번호에 숫자 이외의 문자가 포함된 경우, 오류 메시지를 반환해야 한다", () => {
    const { result } = renderHook(() =>
      usePasswordValidation({ password: "12A" })
    );
    expect(result.current.validationResult.isValid).toBe(false);
    expect(result.current.validationResult.errorMessage).toBe(
  ...
  it("비밀번호의 길이가 3자를 넘어갈 경우, 오류 메시지를 반환해야 한다", () => {
    const { result } = renderHook(() =>
      usePasswordValidation({ password: "1234" })
    );
    expect(result.current.validationResult.isValid).toBe(false);
    expect(result.current.validationResult.errorMessage).toBe(
  ...
  it("비밀번호의 길이가 3자 미만인 경우, 오류 메시지를 반환해야 한다", () => {
    const { result } = renderHook(() =>
      usePasswordValidation({ password: "1" })
    );
    expect(result.current.validationResult.isValid).toBe(false);
    expect(result.current.validationResult.errorMessage).toBe(
      ERROR_MESSAGE.INVALID_PASSWORD_LENGTH
